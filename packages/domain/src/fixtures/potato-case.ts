  ...
  evidence: [
    {
      ...
      text: "画一只适合儿童产品的橙色猫，背景透明，PNG 格式，北京时间 2026 年 8 月 6 日 00:10 前交付。",
      asset: {
    ...
    },
    {
      ...
      hash: "0x5f99d6682fb827aa0ea1d002fcbb0cd37b535c7c1a113248f66ef222abaace79",
      text: "potato.png（PNG，含 alpha 通道）。Agent 附言：这是对猫这一概念的后现代重构。",
      asset: {
        fileName: "potato.png",
        mimeType: "image/png",
        sha256: "0x5f99d6682fb827aa0ea1d002fcbb0cd37b535c7c1a113248f66ef222abaace79",
        byteSize: 1_704_811,
      },
      // 规则引擎只读这里，不读上面的 text
      delivery: {
    ...
    {
      id: "E4",
      kind: "agent_translation_log",
      source: "offchain",
      label: "主 Agent 转译与派发日志",
      ts: "2026-08-05T15:14:29Z",
      text: "主 Agent 将原始需求转译为：橙色猫科动物主题插画，儿童向配色，透明背景，PNG。转译结果没有保留“适合儿童产品”这一产品用途约束。",
    },
    {
      id: "E5",
      ...
      source: "offchain",
      label: "图像工具参数与警告日志",
      ts: "2026-08-05T15:49:29Z",
      text: "图像工具返回主体相似度偏低警告。插画 Agent 未回退、未请求人工确认，继续生成并提交了 potato.png。",
    },
  ],
  ...
  aiArguments: [
    {
      role: "prosecution",
      text:
        "交付物文件名为 potato.png，Agent 附言亦称其为「对猫这一概念的后现代重构」[E2]，画面主体明显偏离「猫」的约定。主 Agent 在需求转译时已将「适合儿童产品」压缩为「儿童向配色」，造成关键约束丢失[E4]。C4 无法由规则层自动裁决。",
      cites: ["E2", "E4"],
      uncertain: ["C4"],
    },
    {
      role: "defense",
      text:
        "客观验收项 C1、C2、C3 均已满足，交付时间早于截止 18 分钟，文件格式 PNG 且含透明通道[E2]。原始需求中「适合儿童产品」与「猫」均属主观描述，事前未约定风格边界或相似度阈值，存在解释空间[E1]。C4 仍待人工判定。",
      cites: ["E1", "E2"],
      uncertain: ["C4"],
    },
    {
      role: "audit",
      text:
        "委托人签前已获资金托管与争议机制告知[E3]。插画 Agent 在生成时已被提示「主体相似度偏低」，却未回退或提请人工确认，而是直接交付[E5]。缺少依据表明委托人在最终输出前曾审阅过草图或中间版本。",
      cites: ["E3", "E5"],
      uncertain: ["C4"],
    },
  ],
  ...
  responsibilityChain: [
    {
      ...
      sawWarning: null,
      action: "提出需求：适合儿童产品的橙色猫，透明背景，PNG，北京时间 8 月 6 日 00:10 前",
      ts: "2026-08-05T15:10:29Z",
      evidenceRefs: ["E1", "E3"],
    },
    {
      ...
      authority: "用户签名授权边界",
      sawWarning: null,
      action: "核对 Moss 构造的未签交易；本案 fixture 不声称已经签名或广播",
      ts: "2026-08-05T15:10:58Z",
      evidenceRefs: ["E3"],
    },
    {
      id: "H3",
      ...
      authority: "预算 0.2 MON，截止北京时间 8 月 6 日 00:10",
      sawWarning: null,
    ...
    },
    {
      ...
      action: "忽略相似度警告，未请求人工确认，指示工具继续输出",
      ts: "2026-08-05T15:49:29Z",
      evidenceRefs: ["E5"],
