...
1. **每一处事实主张后面必须标注证据编号**，写成 \`[E2]\` 或 \`[E1][E2]\` 的形式，并把用到的编号填进 \`cites\`。
   ⚠️ **方括号里只能出现 E 开头的证据编号。**条款编号（C1、C4 等）直接写在句子里，不要加方括号——
   方括号在界面上是可点击的证据角标，点了要能跳到对应证据。
...
**每份意见 2–3 句中文，控制在 120 字以内。**不用标题，不用列表。
三份意见会在界面上并排显示，太长会挤在一起——宁可少说一点，也不要把三个论点塞进一句。`;
